...
static ssize_t pa22x_show_prox_thres_min(struct device_driver *ddri, char *buf)
{
	...
	res = snprintf(buf, PAGE_SIZE, "%d\n", pa22x_obj->crosstalk + PA22X_THRES_MIN);
	return res;
}
...
static ssize_t pa22x_show_prox_thres_max(struct device_driver *ddri, char *buf)
{
	...
	res = snprintf(buf, PAGE_SIZE, "%d\n", pa22x_obj->crosstalk + PA22X_THRES_MAX);
	return res;
}
...
static ssize_t pa22x_show_data_max(struct device_driver *ddri, char *buf)
{
	ssize_t res;
...
static ssize_t pa22x_show_prox_debug(struct device_driver *ddri, char *buf)
{
	ssize_t res;
	...
	res = snprintf(buf, PAGE_SIZE, "%s\n", pa22x_obj->flag_prox_debug? "true" : "false" ); 
	return res;
}

static ssize_t pa22x_store_prox_debug(struct device_driver *ddri, const char *buf, size_t count)
{
	int val = 0;
	...
	if (val) {
		pa22x_obj->flag_prox_debug = true;
	} else {
		pa22x_obj->flag_prox_debug = false;
	}
	return count;
}
...
static DRIVER_ATTR(prox_debug,     S_IWUGO | S_IRUGO, pa22x_show_prox_debug, pa22x_store_prox_debug);
static DRIVER_ATTR(prox_data_max,     S_IWUGO | S_IRUGO, pa22x_show_data_max, NULL);
...
static int pa22x_check_intr(struct i2c_client *client) 
{
	...
	if (obj->flag_prox_debug) {
		APS_LOG("yangkui: now in calibration mode.\n");
		goto check_intr_exit;
	}
	//SUNLIGHT
	if (psdata == 0) 
...
static void pa22x_eint_work(struct work_struct *work)
{
	...
	if(!(obj->hw->polling_mode_ps) && ps_intr_active)
	{
		...
		if(res != 0){
		    ...
		    {
				mutex_lock(&obj->update_lock);
				res = hwmsen_read_byte(obj->client, REG_PS_DATA, &ps_data);
				mutex_unlock(&obj->update_lock);
				sensor_data_ps.values[0] = ps_data+3;
	...
	#if defined(PA22X_NEW_ARCH)
		res = ps_report_interrupt_data(sensor_data_ps.values[0]);
	#else
